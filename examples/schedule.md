...
$availableSlots = Schedule::for($user)
...
$availableSlots = Schedule::for($user)
    ->findAvailableSlots($startDate, $endDate, $duration, 'consultation');
...
$emergencySlots = Schedule::for($user)
    ->findAvailableSlots($startDate, $endDate, 15, 'emergency'); // 15 minutes
```
...
class ScheduleController extends Controller
{
     ...
     */
    public function findAvailableSlots(User $user, Request $request): JsonResponse
    {
        $request->validate([
        ...
        $slots = Schedule::for($user)
            ->findAvailableSlots($startDate, $endDate, $duration, $type);
     ...
     */
    public function nextAvailableSlot(User $user, Request $request): JsonResponse
    {
        $request->validate([
...
class AppointmentService
{
    ...
    public function findAlternativeSlots(User $doctor, array $originalRequest): array
    {
        ...
        $slotsToday = Schedule::for($doctor)
            ->findAvailableSlots($today, $today->copy()->endOfDay(), $duration, $type);

        // Créneaux disponibles demain
        $slotsTomorrow = Schedule::for($doctor)
            ->findAvailableSlots($tomorrow, $tomorrow->copy()->endOfDay(), $duration, $type);
